...
- 39-dars: Virtual Environment va pip
...
`Django` nima ekanini sodda tilda tushuntiramiz.

Masalan:

> `Django` — bu "Batareyasi ichida" (Batteries included) uslubidagi freymvork bo'lib, o'z ichida admin panel, ORM, forma validation va xavfsizlik vositalarini tayyor taqdim etadi.
...
for num, txt in FULL_LONG_40_TO_48.items():
    try:
...
for item in j_data:
    ...
    if n in FULL_LONG_40_TO_48:
        item["content"] = FULL_LONG_40_TO_48[n]

with open(LESSONS_JSON, "w", encoding="utf-8") as f:
    json.dump(j_data, f, ensure_ascii=False, indent=2)

print("✨ 40-48 darslar kontenti to'liqligicha 10 bo'limli variantda saqlandi!")
