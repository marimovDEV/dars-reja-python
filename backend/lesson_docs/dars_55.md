# 📝 55. Django Templates (DTL), Variable va Filterlar — Dars dokumentatsiyasi

Django Templates (DTL), Variable va Filterlar — bu zamonaviy dasturlash sohasidagi eng muhim va talab yuqori bo'lgan texnologiyalardan biridir. Ushbu dars davomida biz DTL, {{ var }}, {% if %}, {% for %}, {% extends %} mavzusini eng mayda detallarigacha chuqur va amaliy o'rganamiz.
...
- Django Templates (DTL), Variable va Filterlar texnologiyalarini noldan professional darajada egallash;
- Loyiha kodida DTL, {{ var }}, {% if %}, {% for %}, {% extends %} standartlarini to'liq qo'llash;
...
- Django Templates (DTL), Variable va Filterlar tushunchasining arxitekturaviy mohiyatini teran anglaydi;
...
- 54-dars bilimlari va amaliy ko'nikmalari;
...
> **Django Texnologiyasi** — DTL, {{ var }}, {% if %}, {% for %}, {% extends %} jarayonlarini tashkil etuvchi asosiy vosita.
...
```python
# Django Templates (DTL), Variable va Filterlar — Standart Sintaksis Misoli
...
Ushbu sintaksis Django Templates (DTL), Variable va Filterlar doirasida ma'lumotlarni tekshirish va to'g'ri qayta ishlashning tayanch namunasidir.
...
### Misol 1 — Django Templates (DTL), Variable va Filterlar — Boshlang'ich Konfiguratsiya

```python
# 55-dars: Birinchi bosqich
print('=== Django Templates (DTL), Variable va Filterlar ===')
data = {'lesson': 55, 'topic': 'Django Templates (DTL), Variable va Filterlar'}
print(data)
```

**Natija:**
```text
=== Django Templates (DTL), Variable va Filterlar ===
{'lesson': 55, 'topic': 'Django Templates (DTL), Variable va Filterlar'}
```
...
def final_summary():
    return {
        'lesson': 55,
        'title': 'Django Templates (DTL), Variable va Filterlar',
        'status': 'Completed',
        'quality': '100%'
    }
print(final_summary())
...
**Natija:**
```text
{'lesson': 55, 'title': 'Django Templates (DTL), Variable va Filterlar', 'status': 'Completed', 'quality': '100%'}
```
...
## Real loyiha misoli: Real Loyiha: Django Templates (DTL), Variable va Filterlar Boshqaruv Tizimi Module #55

```python
# 55-DARS: REAL PROFESSIONAL LOYIHA KODI
import sys
import logging
...
if __name__ == '__main__':
    manager = LessonManager(55, "Django Templates (DTL), Variable va Filterlar")
    manager.add_record({'id': 1, 'name': 'Modul Sozlamalari'})
    manager.add_record({'id': 2, 'name': 'Production Integratsiya'})
    print(manager.get_summary())
```

Ushbu real loyiha kodi Django Templates (DTL), Variable va Filterlar mavzusining amaliy qo'llanilishini aks ettiradi.
...
### 1-topshiriq — Oson
**Vazifa:** Django Templates (DTL), Variable va Filterlar bo'yicha asosiy sintaksis va tayanch funksiyani yozing.

### 2-topshiriq — O'rta
**Vazifa:** Django Templates (DTL), Variable va Filterlar imkoniyatlaridan foydalanib, ma'lumotlarni filtrlovchi dastur tuzing.

### 3-topshiriq — Murakkab
**Vazifa:** Django Templates (DTL), Variable va Filterlar va oldingi darslar bilimlarini birlashtirgan holda to'liq loyiha modulini yozing va test qiling.

## Uyga vazifa

1. Django Templates (DTL), Variable va Filterlar mavzusidagi barcha nazariy bilimlarni takrorlash;
...
1. Django Templates (DTL), Variable va Filterlar ning asosiy vazifasi nimadan iborat?
...
**1.** Ushbu texnologiya DTL, {{ var }}, {% if %}, {% for %}, {% extends %} jarayonlarini avtomatlashtirish va xavfsizligini ta'minlash uchun xizmat qiladi.
...
- Django Templates (DTL), Variable va Filterlar texnologiyasi asoslari;
...
Keyingi **56-dars: 56-dars: Django Static Files & Media Fayllar** da ushbu bilimlar yanada chuqurlashtiriladi.

## Qo'shimcha resurslar

- 📄 Rasmiy hujjat: https://docs.python.org/3/
- 📄 Qo'shimcha qo'llanma: https://marimovdev.uz/docs/55
- 💻 Kodlar: github.com/dars-reja/55-dars
- 🎬 Video dars: youtube.com/watch?v=dars55