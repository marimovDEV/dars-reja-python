# 🔑 65. JWT (JSON Web Tokens) Authentication va SimpleJWT — Dars dokumentatsiyasi

JWT (JSON Web Tokens) Authentication va SimpleJWT — bu zamonaviy dasturlash sohasidagi eng muhim va talab yuqori bo'lgan texnologiyalardan biridir. Ushbu dars davomida biz SimpleJWT, token_obtain_pair, Refresh/Access Tokens mavzusini eng mayda detallarigacha chuqur va amaliy o'rganamiz.
...
- JWT (JSON Web Tokens) Authentication va SimpleJWT texnologiyalarini noldan professional darajada egallash;
- Loyiha kodida SimpleJWT, token_obtain_pair, Refresh/Access Tokens standartlarini to'liq qo'llash;
...
- JWT (JSON Web Tokens) Authentication va SimpleJWT tushunchasining arxitekturaviy mohiyatini teran anglaydi;
...
- 64-dars bilimlari va amaliy ko'nikmalari;
...
## JWT Texnologiyasi nima?

> **JWT Texnologiyasi** — SimpleJWT, token_obtain_pair, Refresh/Access Tokens jarayonlarini tashkil etuvchi asosiy vosita.
...
```python
# JWT (JSON Web Tokens) Authentication va SimpleJWT — Standart Sintaksis Misoli
...
Ushbu sintaksis JWT (JSON Web Tokens) Authentication va SimpleJWT doirasida ma'lumotlarni tekshirish va to'g'ri qayta ishlashning tayanch namunasidir.
...
### Misol 1 — JWT (JSON Web Tokens) Authentication va SimpleJWT — Boshlang'ich Konfiguratsiya

```python
# 65-dars: Birinchi bosqich
print('=== JWT (JSON Web Tokens) Authentication va SimpleJWT ===')
data = {'lesson': 65, 'topic': 'JWT (JSON Web Tokens) Authentication va SimpleJWT'}
print(data)
```

**Natija:**
```text
=== JWT (JSON Web Tokens) Authentication va SimpleJWT ===
{'lesson': 65, 'topic': 'JWT (JSON Web Tokens) Authentication va SimpleJWT'}
```
...
def final_summary():
    return {
        'lesson': 65,
        'title': 'JWT (JSON Web Tokens) Authentication va SimpleJWT',
        'status': 'Completed',
        'quality': '100%'
    }
print(final_summary())
...
**Natija:**
```text
{'lesson': 65, 'title': 'JWT (JSON Web Tokens) Authentication va SimpleJWT', 'status': 'Completed', 'quality': '100%'}
```
...
## Real loyiha misoli: Real Loyiha: JWT (JSON Web Tokens) Authentication va SimpleJWT Boshqaruv Tizimi Module #65

```python
# 65-DARS: REAL PROFESSIONAL LOYIHA KODI
import sys
import logging
...
if __name__ == '__main__':
    manager = LessonManager(65, "JWT (JSON Web Tokens) Authentication va SimpleJWT")
    manager.add_record({'id': 1, 'name': 'Modul Sozlamalari'})
    manager.add_record({'id': 2, 'name': 'Production Integratsiya'})
    print(manager.get_summary())
```

Ushbu real loyiha kodi JWT (JSON Web Tokens) Authentication va SimpleJWT mavzusining amaliy qo'llanilishini aks ettiradi.
...
### 1-topshiriq — Oson
**Vazifa:** JWT (JSON Web Tokens) Authentication va SimpleJWT bo'yicha asosiy sintaksis va tayanch funksiyani yozing.

### 2-topshiriq — O'rta
**Vazifa:** JWT (JSON Web Tokens) Authentication va SimpleJWT imkoniyatlaridan foydalanib, ma'lumotlarni filtrlovchi dastur tuzing.

### 3-topshiriq — Murakkab
**Vazifa:** JWT (JSON Web Tokens) Authentication va SimpleJWT va oldingi darslar bilimlarini birlashtirgan holda to'liq loyiha modulini yozing va test qiling.

## Uyga vazifa

1. JWT (JSON Web Tokens) Authentication va SimpleJWT mavzusidagi barcha nazariy bilimlarni takrorlash;
...
1. JWT (JSON Web Tokens) Authentication va SimpleJWT ning asosiy vazifasi nimadan iborat?
...
**1.** Ushbu texnologiya SimpleJWT, token_obtain_pair, Refresh/Access Tokens jarayonlarini avtomatlashtirish va xavfsizligini ta'minlash uchun xizmat qiladi.
...
- JWT (JSON Web Tokens) Authentication va SimpleJWT texnologiyasi asoslari;
...
Keyingi **66-dars: 66-dars: DRF Filtering, Searching va Pagination** da ushbu bilimlar yanada chuqurlashtiriladi.

## Qo'shimcha resurslar

- 📄 Rasmiy hujjat: https://docs.python.org/3/
- 📄 Qo'shimcha qo'llanma: https://marimovdev.uz/docs/65
- 💻 Kodlar: github.com/dars-reja/65-dars
- 🎬 Video dars: youtube.com/watch?v=dars65