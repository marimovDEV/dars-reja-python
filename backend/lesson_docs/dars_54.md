# 🏛 54. Django Apps va MVT (Model-View-Template) Arxitekturasi — Dars dokumentatsiyasi

Django Apps va MVT (Model-View-Template) Arxitekturasi — bu zamonaviy dasturlash sohasidagi eng muhim va talab yuqori bo'lgan texnologiyalardan biridir. Ushbu dars davomida biz startapp, MVT, views.py va urls.py mavzusini eng mayda detallarigacha chuqur va amaliy o'rganamiz.
...
- Django Apps va MVT (Model-View-Template) Arxitekturasi texnologiyalarini noldan professional darajada egallash;
- Loyiha kodida startapp, MVT, views.py va urls.py standartlarini to'liq qo'llash;
...
- Django Apps va MVT (Model-View-Template) Arxitekturasi tushunchasining arxitekturaviy mohiyatini teran anglaydi;
...
- 53-dars bilimlari va amaliy ko'nikmalari;
...
## Django Texnologiyasi nima?

> **Django Texnologiyasi** — startapp, MVT, views.py va urls.py jarayonlarini tashkil etuvchi asosiy vosita.
...
```python
# Django Apps va MVT (Model-View-Template) Arxitekturasi — Standart Sintaksis Misoli
...
Ushbu sintaksis Django Apps va MVT (Model-View-Template) Arxitekturasi doirasida ma'lumotlarni tekshirish va to'g'ri qayta ishlashning tayanch namunasidir.
...
### Misol 1 — Django Apps va MVT (Model-View-Template) Arxitekturasi — Boshlang'ich Konfiguratsiya

```python
# 54-dars: Birinchi bosqich
print('=== Django Apps va MVT (Model-View-Template) Arxitekturasi ===')
data = {'lesson': 54, 'topic': 'Django Apps va MVT (Model-View-Template) Arxitekturasi'}
print(data)
```

**Natija:**
```text
=== Django Apps va MVT (Model-View-Template) Arxitekturasi ===
{'lesson': 54, 'topic': 'Django Apps va MVT (Model-View-Template) Arxitekturasi'}
```
...
def final_summary():
    return {
        'lesson': 54,
        'title': 'Django Apps va MVT (Model-View-Template) Arxitekturasi',
        'status': 'Completed',
        'quality': '100%'
    }
print(final_summary())
...
**Natija:**
```text
{'lesson': 54, 'title': 'Django Apps va MVT (Model-View-Template) Arxitekturasi', 'status': 'Completed', 'quality': '100%'}
```
...
## Real loyiha misoli: Real Loyiha: Django Apps va MVT (Model-View-Template) Arxitekturasi Boshqaruv Tizimi Module #54

```python
# 54-DARS: REAL PROFESSIONAL LOYIHA KODI
import sys
import logging
...
if __name__ == '__main__':
    manager = LessonManager(54, "Django Apps va MVT (Model-View-Template) Arxitekturasi")
    manager.add_record({'id': 1, 'name': 'Modul Sozlamalari'})
    manager.add_record({'id': 2, 'name': 'Production Integratsiya'})
    print(manager.get_summary())
```

Ushbu real loyiha kodi Django Apps va MVT (Model-View-Template) Arxitekturasi mavzusining amaliy qo'llanilishini aks ettiradi.
...
### 1-topshiriq — Oson
**Vazifa:** Django Apps va MVT (Model-View-Template) Arxitekturasi bo'yicha asosiy sintaksis va tayanch funksiyani yozing.

### 2-topshiriq — O'rta
**Vazifa:** Django Apps va MVT (Model-View-Template) Arxitekturasi imkoniyatlaridan foydalanib, ma'lumotlarni filtrlovchi dastur tuzing.

### 3-topshiriq — Murakkab
**Vazifa:** Django Apps va MVT (Model-View-Template) Arxitekturasi va oldingi darslar bilimlarini birlashtirgan holda to'liq loyiha modulini yozing va test qiling.

## Uyga vazifa

1. Django Apps va MVT (Model-View-Template) Arxitekturasi mavzusidagi barcha nazariy bilimlarni takrorlash;
...
1. Django Apps va MVT (Model-View-Template) Arxitekturasi ning asosiy vazifasi nimadan iborat?
...
**1.** Ushbu texnologiya startapp, MVT, views.py va urls.py jarayonlarini avtomatlashtirish va xavfsizligini ta'minlash uchun xizmat qiladi.
...
- Django Apps va MVT (Model-View-Template) Arxitekturasi texnologiyasi asoslari;
...
Keyingi **55-dars: 55-dars: Django Templates (DTL), Variable va Filterlar** da ushbu bilimlar yanada chuqurlashtiriladi.

## Qo'shimcha resurslar

- 📄 Rasmiy hujjat: https://docs.python.org/3/
- 📄 Qo'shimcha qo'llanma: https://marimovdev.uz/docs/54
- 💻 Kodlar: github.com/dars-reja/54-dars
- 🎬 Video dars: youtube.com/watch?v=dars54