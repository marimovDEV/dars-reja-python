# ⚡ 68. Celery va Redis bilan Asinxron Vazifalar — Dars dokumentatsiyasi

Celery va Redis bilan Asinxron Vazifalar — bu zamonaviy dasturlash sohasidagi eng muhim va talab yuqori bo'lgan texnologiyalardan biridir. Ushbu dars davomida biz Celery worker, Redis broker, shared_task, Background tasks mavzusini eng mayda detallarigacha chuqur va amaliy o'rganamiz.
...
- Celery va Redis bilan Asinxron Vazifalar texnologiyalarini noldan professional darajada egallash;
- Loyiha kodida Celery worker, Redis broker, shared_task, Background tasks standartlarini to'liq qo'llash;
...
- Celery va Redis bilan Asinxron Vazifalar tushunchasining arxitekturaviy mohiyatini teran anglaydi;
...
- 67-dars bilimlari va amaliy ko'nikmalari;
...
## Celery Texnologiyasi nima?

> **Celery Texnologiyasi** — Celery worker, Redis broker, shared_task, Background tasks jarayonlarini tashkil etuvchi asosiy vosita.
...
```python
# Celery va Redis bilan Asinxron Vazifalar — Standart Sintaksis Misoli
...
Ushbu sintaksis Celery va Redis bilan Asinxron Vazifalar doirasida ma'lumotlarni tekshirish va to'g'ri qayta ishlashning tayanch namunasidir.
...
### Misol 1 — Celery va Redis bilan Asinxron Vazifalar — Boshlang'ich Konfiguratsiya

```python
# 68-dars: Birinchi bosqich
print('=== Celery va Redis bilan Asinxron Vazifalar ===')
data = {'lesson': 68, 'topic': 'Celery va Redis bilan Asinxron Vazifalar'}
print(data)
```

**Natija:**
```text
=== Celery va Redis bilan Asinxron Vazifalar ===
{'lesson': 68, 'topic': 'Celery va Redis bilan Asinxron Vazifalar'}
```
...
def final_summary():
    return {
        'lesson': 68,
        'title': 'Celery va Redis bilan Asinxron Vazifalar',
        'status': 'Completed',
        'quality': '100%'
    }
print(final_summary())
...
**Natija:**
```text
{'lesson': 68, 'title': 'Celery va Redis bilan Asinxron Vazifalar', 'status': 'Completed', 'quality': '100%'}
```
...
## Real loyiha misoli: Real Loyiha: Celery va Redis bilan Asinxron Vazifalar Boshqaruv Tizimi Module #68

```python
# 68-DARS: REAL PROFESSIONAL LOYIHA KODI
import sys
import logging
...
if __name__ == '__main__':
    manager = LessonManager(68, "Celery va Redis bilan Asinxron Vazifalar")
    manager.add_record({'id': 1, 'name': 'Modul Sozlamalari'})
    manager.add_record({'id': 2, 'name': 'Production Integratsiya'})
    print(manager.get_summary())
```

Ushbu real loyiha kodi Celery va Redis bilan Asinxron Vazifalar mavzusining amaliy qo'llanilishini aks ettiradi.
...
### 1-topshiriq — Oson
**Vazifa:** Celery va Redis bilan Asinxron Vazifalar bo'yicha asosiy sintaksis va tayanch funksiyani yozing.

### 2-topshiriq — O'rta
**Vazifa:** Celery va Redis bilan Asinxron Vazifalar imkoniyatlaridan foydalanib, ma'lumotlarni filtrlovchi dastur tuzing.

### 3-topshiriq — Murakkab
**Vazifa:** Celery va Redis bilan Asinxron Vazifalar va oldingi darslar bilimlarini birlashtirgan holda to'liq loyiha modulini yozing va test qiling.

## Uyga vazifa

1. Celery va Redis bilan Asinxron Vazifalar mavzusidagi barcha nazariy bilimlarni takrorlash;
...
1. Celery va Redis bilan Asinxron Vazifalar ning asosiy vazifasi nimadan iborat?
...
**1.** Ushbu texnologiya Celery worker, Redis broker, shared_task, Background tasks jarayonlarini avtomatlashtirish va xavfsizligini ta'minlash uchun xizmat qiladi.
...
- Celery va Redis bilan Asinxron Vazifalar texnologiyasi asoslari;
...
Keyingi **69-dars: 69-dars: Docker va Docker Compose Konteynerlashtirish** da ushbu bilimlar yanada chuqurlashtiriladi.

## Qo'shimcha resurslar

- 📄 Rasmiy hujjat: https://docs.python.org/3/
- 📄 Qo'shimcha qo'llanma: https://marimovdev.uz/docs/68
- 💻 Kodlar: github.com/dars-reja/68-dars
- 🎬 Video dars: youtube.com/watch?v=dars68