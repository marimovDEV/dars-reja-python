# 🐳 69. Docker va Docker Compose bilan Konteynerlashtirish — Dars dokumentatsiyasi

Docker va Docker Compose bilan Konteynerlashtirish — bu zamonaviy dasturlash sohasidagi eng muhim va talab yuqori bo'lgan texnologiyalardan biridir. Ushbu dars davomida biz Dockerfile, docker-compose.yml, Django, Postgres, Redis mavzusini eng mayda detallarigacha chuqur va amaliy o'rganamiz.
...
- Docker va Docker Compose bilan Konteynerlashtirish texnologiyalarini noldan professional darajada egallash;
- Loyiha kodida Dockerfile, docker-compose.yml, Django, Postgres, Redis standartlarini to'liq qo'llash;
...
- Docker va Docker Compose bilan Konteynerlashtirish tushunchasining arxitekturaviy mohiyatini teran anglaydi;
...
- 68-dars bilimlari va amaliy ko'nikmalari;
...
## Docker Texnologiyasi nima?

> **Docker Texnologiyasi** — Dockerfile, docker-compose.yml, Django, Postgres, Redis jarayonlarini tashkil etuvchi asosiy vosita.
...
```python
# Docker va Docker Compose bilan Konteynerlashtirish — Standart Sintaksis Misoli
...
Ushbu sintaksis Docker va Docker Compose bilan Konteynerlashtirish doirasida ma'lumotlarni tekshirish va to'g'ri qayta ishlashning tayanch namunasidir.
...
### Misol 1 — Docker va Docker Compose bilan Konteynerlashtirish — Boshlang'ich Konfiguratsiya

```python
# 69-dars: Birinchi bosqich
print('=== Docker va Docker Compose bilan Konteynerlashtirish ===')
data = {'lesson': 69, 'topic': 'Docker va Docker Compose bilan Konteynerlashtirish'}
print(data)
```

**Natija:**
```text
=== Docker va Docker Compose bilan Konteynerlashtirish ===
{'lesson': 69, 'topic': 'Docker va Docker Compose bilan Konteynerlashtirish'}
```
...
def final_summary():
    return {
        'lesson': 69,
        'title': 'Docker va Docker Compose bilan Konteynerlashtirish',
        'status': 'Completed',
        'quality': '100%'
    }
print(final_summary())
...
**Natija:**
```text
{'lesson': 69, 'title': 'Docker va Docker Compose bilan Konteynerlashtirish', 'status': 'Completed', 'quality': '100%'}
```
...
## Real loyiha misoli: Real Loyiha: Docker va Docker Compose bilan Konteynerlashtirish Boshqaruv Tizimi Module #69

```python
# 69-DARS: REAL PROFESSIONAL LOYIHA KODI
import sys
import logging
...
if __name__ == '__main__':
    manager = LessonManager(69, "Docker va Docker Compose bilan Konteynerlashtirish")
    manager.add_record({'id': 1, 'name': 'Modul Sozlamalari'})
    manager.add_record({'id': 2, 'name': 'Production Integratsiya'})
    print(manager.get_summary())
```

Ushbu real loyiha kodi Docker va Docker Compose bilan Konteynerlashtirish mavzusining amaliy qo'llanilishini aks ettiradi.
...
### 1-topshiriq — Oson
**Vazifa:** Docker va Docker Compose bilan Konteynerlashtirish bo'yicha asosiy sintaksis va tayanch funksiyani yozing.

### 2-topshiriq — O'rta
**Vazifa:** Docker va Docker Compose bilan Konteynerlashtirish imkoniyatlaridan foydalanib, ma'lumotlarni filtrlovchi dastur tuzing.

### 3-topshiriq — Murakkab
**Vazifa:** Docker va Docker Compose bilan Konteynerlashtirish va oldingi darslar bilimlarini birlashtirgan holda to'liq loyiha modulini yozing va test qiling.

## Uyga vazifa

1. Docker va Docker Compose bilan Konteynerlashtirish mavzusidagi barcha nazariy bilimlarni takrorlash;
...
1. Docker va Docker Compose bilan Konteynerlashtirish ning asosiy vazifasi nimadan iborat?
...
**1.** Ushbu texnologiya Dockerfile, docker-compose.yml, Django, Postgres, Redis jarayonlarini avtomatlashtirish va xavfsizligini ta'minlash uchun xizmat qiladi.
...
- Docker va Docker Compose bilan Konteynerlashtirish texnologiyasi asoslari;
...
Keyingi **70-dars: 70-dars: Production Server Sozlash (Nginx, Gunicorn, VPS)** da ushbu bilimlar yanada chuqurlashtiriladi.

## Qo'shimcha resurslar

- 📄 Rasmiy hujjat: https://docs.python.org/3/
- 📄 Qo'shimcha qo'llanma: https://marimovdev.uz/docs/69
- 💻 Kodlar: github.com/dars-reja/69-dars
- 🎬 Video dars: youtube.com/watch?v=dars69