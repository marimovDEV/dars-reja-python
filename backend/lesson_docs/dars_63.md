# 🏗️ 63. DRF Class-Based Views (CBV) va Generic Views — Dars dokumentatsiyasi

DRF Class-Based Views (CBV) va Generic Views — bu zamonaviy dasturlash sohasidagi eng muhim va talab yuqori bo'lgan texnologiyalardan biridir. Ushbu dars davomida biz APIView, ListCreateAPIView, ModelViewSet, Router mavzusini eng mayda detallarigacha chuqur va amaliy o'rganamiz.
...
- DRF Class-Based Views (CBV) va Generic Views texnologiyalarini noldan professional darajada egallash;
- Loyiha kodida APIView, ListCreateAPIView, ModelViewSet, Router standartlarini to'liq qo'llash;
...
- DRF Class-Based Views (CBV) va Generic Views tushunchasining arxitekturaviy mohiyatini teran anglaydi;
...
- 62-dars bilimlari va amaliy ko'nikmalari;
...
> **DRF Texnologiyasi** — APIView, ListCreateAPIView, ModelViewSet, Router jarayonlarini tashkil etuvchi asosiy vosita.
...
```python
# DRF Class-Based Views (CBV) va Generic Views — Standart Sintaksis Misoli
...
Ushbu sintaksis DRF Class-Based Views (CBV) va Generic Views doirasida ma'lumotlarni tekshirish va to'g'ri qayta ishlashning tayanch namunasidir.
...
### Misol 1 — DRF Class-Based Views (CBV) va Generic Views — Boshlang'ich Konfiguratsiya

```python
# 63-dars: Birinchi bosqich
print('=== DRF Class-Based Views (CBV) va Generic Views ===')
data = {'lesson': 63, 'topic': 'DRF Class-Based Views (CBV) va Generic Views'}
print(data)
```

**Natija:**
```text
=== DRF Class-Based Views (CBV) va Generic Views ===
{'lesson': 63, 'topic': 'DRF Class-Based Views (CBV) va Generic Views'}
```
...
**Natija:**
```text
{'lesson': 63, 'title': 'DRF Class-Based Views (CBV) va Generic Views', 'status': 'Completed', 'quality': '100%'}
```
...
## Real loyiha misoli: Real Loyiha: DRF Class-Based Views (CBV) va Generic Views Boshqaruv Tizimi Module #63

```python
# 63-DARS: REAL PROFESSIONAL LOYIHA KODI
import sys
import logging
...
if __name__ == '__main__':
    manager = LessonManager(63, "DRF Class-Based Views (CBV) va Generic Views")
    manager.add_record({'id': 1, 'name': 'Modul Sozlamalari'})
    manager.add_record({'id': 2, 'name': 'Production Integratsiya'})
    print(manager.get_summary())
```

Ushbu real loyiha kodi DRF Class-Based Views (CBV) va Generic Views mavzusining amaliy qo'llanilishini aks ettiradi.
...
### 1-topshiriq — Oson
**Vazifa:** DRF Class-Based Views (CBV) va Generic Views bo'yicha asosiy sintaksis va tayanch funksiyani yozing.

### 2-topshiriq — O'rta
**Vazifa:** DRF Class-Based Views (CBV) va Generic Views imkoniyatlaridan foydalanib, ma'lumotlarni filtrlovchi dastur tuzing.

### 3-topshiriq — Murakkab
**Vazifa:** DRF Class-Based Views (CBV) va Generic Views va oldingi darslar bilimlarini birlashtirgan holda to'liq loyiha modulini yozing va test qiling.

## Uyga vazifa

1. DRF Class-Based Views (CBV) va Generic Views mavzusidagi barcha nazariy bilimlarni takrorlash;
...
1. DRF Class-Based Views (CBV) va Generic Views ning asosiy vazifasi nimadan iborat?
...
**1.** Ushbu texnologiya APIView, ListCreateAPIView, ModelViewSet, Router jarayonlarini avtomatlashtirish va xavfsizligini ta'minlash uchun xizmat qiladi.
...
- DRF Class-Based Views (CBV) va Generic Views texnologiyasi asoslari;
...
Keyingi **64-dars: 64-dars: DRF Authentication va Permissions** da ushbu bilimlar yanada chuqurlashtiriladi.

## Qo'shimcha resurslar

- 📄 Rasmiy hujjat: https://docs.python.org/3/
- 📄 Qo'shimcha qo'llanma: https://marimovdev.uz/docs/63
- 💻 Kodlar: github.com/dars-reja/63-dars
- 🎬 Video dars: youtube.com/watch?v=dars63